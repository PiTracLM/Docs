...
- **Check for Updates** -- always available. Runs `git fetch` against `origin` and counts commits on the selected branch that are not yet in your local `HEAD`.
- **Update & Rebuild** -- shown when the selected branch matches the current branch. Enabled only after a check reports new commits. Runs `git pull --ff-only` and then `sudo build.sh dev` (incremental build).
...
3. `sudo build.sh dev` (or `dev force` for a branch switch / forced rebuild) runs. `build.sh` itself restarts the `pitrac-web` service when it finishes.
